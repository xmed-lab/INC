...
class Distortions(object):
    ...
    def marking_blur(self, x, severity=1):
        x = np.array(x)
        blot = dict(self.blots[0]['224'][severity-1])
        blur = blot["blur"]
        binary = blot["mask"]
    ...
    def bubble_blur(self, x, severity=1):
        ...
        x = np.array(x)
        sample = dict(self.bubbles[0]['224'])
...
class RandomDistortions(torch.nn.Module):
    ...
    def forward(self, img):
        distortion = getattr(self.distor, random.choice(self.distortions))
        severity = random.choice([1, 2, 3, 4, 5])
 
        # for multi-crop, only one aug inside (no multiple corruptions for one patch)
        if isinstance(img, tuple):
            out = []
            for _ in img:
                out.append(distortion(_, severity))
            return out
        else:
            return distortion(img, severity)
