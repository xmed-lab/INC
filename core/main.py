...
def INC(args, gallery_feats, gallery_labels, gallery_patch_names, 
    ...
    ## wo. relative patch
    #query_logits = pos_cosine.mean(0) - neg_cosine.mean(0)
    #wsi_pred, query_idxs = query_logits.topk(min(top_instance, len(query_patch_names)))
    #wsi_pred = wsi_pred.mean()
    ...
    for i in range(args.runs):
        ...
        for n in all_query_names:
            ...
            if 'prototype' in mode:
                ...
                # experiments suggest prototype suits "pos logit" better than "knn prob"
                topk = min(args.top_instance, query_feats.shape[0])
                #prob = query_feats @ gallery_feats[0] - query_feats @ gallery_feats[1]
                #wsi_pred = prob.topk(topk)[0].mean() # pos-neg logit

                prob = query_feats @ gallery_feats[0]
                wsi_pred = prob.topk(topk)[0].mean() # pos logit (final)
                
                #sim = query_feats @ gallery_feats[0] #positive similarity as logit to select top query instances
                #top_patch_idx = sim.topk(topk)[1]
                #label_idxs = (query_feats[top_patch_idx, :] @ gallery_feats.t()).argmax(1)
                #wsi_pred = gallery_labels[label_idxs].float().mean() # knn prob

                #sim = query_feats @ gallery_feats[0] #positive similarity as logit to select top query instances
                #top_patch_idx = sim.topk(topk)[1]
                #label_idxs = (query_feats[top_patch_idx, :] @ gallery_feats.t()).argmax(1)
                #wsi_pred = (gallery_labels[label_idxs].float().mean() > 0.5).float() # knn binary (logit avg, voting)

                #sim = query_feats @ gallery_feats[0] #positive similarity as logit to select top query instances
                #top_patch_idx = sim.topk(topk)[1]
                #wsi_pred = (query_feats[top_patch_idx, :].mean(0) @ gallery_feats.t()).argmax(0) # knn binary (query feat avg)
                ...
                # experiments suggest knn prob suits KNN better than pos logit
                #_, label_idxs = (gallery_feats @ query_feats).topk(min(5, gallery_feats.shape[0]))
                #wsi_pred = gallery_labels[label_idxs].float().mean() # knn prob
                ...
                wsi_pred = (pos_gallery_feats @ query_feats).topk(min(5, pos_gallery_feats.shape[0]))[0].mean() - \
                        (neg_gallery_feats @ query_feats).topk(min(5, neg_gallery_feats.shape[0]))[0].mean()# pos-neg logit (final)

                #pos_gallery_feats = gallery_feats[gallery_labels == 1]
                #wsi_pred = (pos_gallery_feats @ query_feats).topk(min(5, pos_gallery_feats.shape[0]))[0].mean() # pos logit

                #_, label_idxs = (gallery_feats @ query_feats).topk(min(5, gallery_feats.shape[0]))
                #wsi_pred = (gallery_labels[label_idxs].float().mean() > 0.5).float() # knn binary
